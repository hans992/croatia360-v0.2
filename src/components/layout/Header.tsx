...
import { User, Menu, X, ShipWheel } from 'lucide-react';
  ...
  return (
    <span className="flex items-center gap-2.5">
      ...
      <span className="font-heading text-lg font-extrabold tracking-tight text-foreground">
        Adriatic<span className="text-primary">ByBoat</span>
      </span>
    </span>
  );
}
...
const Header = ({ locale }: HeaderProps) => {
  ...
  const marketplaceCopy = getMarketplaceCopy(currentLocale);
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (!user && session) setUser(session.user as SupabaseUser | null ?? null);
      if (!authChecked) setAuthChecked(true);
    });
  ...
  const navLinks: NavLinkItem[] = [
    { href: '/zadar/boat-tours', label: marketplaceCopy.navBoatTours, isPrimary: true },
    { href: '/explore', labelKey: 'header_explore' },
    { href: '/', labelKey: 'header_sara_ai' },
  ];
  ...
  return (
    <header className={`sticky w-full z-50 glass-panel h-16 transition-all duration-normal ease-out ${isChatPage ? 'top-0' : (scrollDirection === 'down' ? '-top-16' : 'top-0')}`}>
      <div className="container mx-auto flex h-full items-center justify-between px-4">
        <Link href={`/${currentLocale}/`} aria-label={`${BRAND.name} home`} className="group">
          <BrandMark />
        </Link>
        ...
        <nav className="hidden md:flex items-center space-x-8 text-sm font-medium">
          ...
          {userNavLinks.map((link) => <Link key={link.href} href={`/${currentLocale}${link.href}`} className="transition-colors hover:text-primary text-foreground/80">{resolveLabel(link)}</Link>)}
        </nav>

        <div className="hidden md:flex items-center space-x-2 sm:space-x-4">
          <LanguageSwitcher currentLocale={currentLocale} />
          <ThemeSwitcher />
          {!authChecked ? <div className="h-8 w-24 animate-pulse bg-muted rounded-md" /> : user ? (
            ...
            <Button asChild variant="default" className="bg-primary hover:bg-primary/90 text-primary-foreground"><Link href={`/${currentLocale}/login`}><User className="h-5 w-5 mr-2" />{t('header_login')}</Link></Button>
          )}
        </div>
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-[300px] sm:w-[320px] bg-background text-foreground p-0 flex flex-col">
              <div className="flex justify-between items-center p-4 border-b border-border">
                <Link href={`/${currentLocale}/`} onClick={() => setIsMobileMenuOpen(false)}><BrandMark /></Link>
              ...
              <div className="p-4 border-t border-border">
                {!authChecked ? <div className="h-8 w-full animate-pulse bg-muted rounded-md mb-2" /> : user ? (
                  <div className="flex flex-col space-y-2"><span className="text-sm font-medium px-3 py-2 text-foreground/80">{t('logged_in_as') || 'Logged in as:'} {displayName}</span><Button variant="outline" size="sm" className="w-full border-destructive text-destructive hover:bg-destructive/10" onClick={async () => { await supabase.auth.signOut(); setUser(null); setIsMobileMenuOpen(false); }}>{t('header_logout')}</Button></div>
                ) : (
                  <SheetClose asChild><Button asChild variant="default" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground"><Link href={`/${currentLocale}/login`} className="w-full"><User className="h-5 w-5 mr-2" />{t('login_register_button') || 'Login / Register'}</Link></Button></SheetClose>
                )}
              </div>
            </SheetContent>
          </Sheet>
