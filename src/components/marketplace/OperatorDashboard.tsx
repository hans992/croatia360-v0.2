...
export default function OperatorDashboard({ locale }: { locale: string }) {
  ...
  async function updateInquiry(id: string, status: 'accepted' | 'declined' | 'contacted') {
    ...
    try {
      const response = await fetch('/api/marketplace/operator/inquiries/decision', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ inquiryId: id, decision: status }),
      });
      const result = await response.json();
      if (!response.ok) {
        setError(result.error ?? 'Could not update booking request.');
      } else {
        if (result.notificationWarning) setNotice(result.notificationWarning);
        else if (status !== 'contacted' && result.notificationSent) setNotice('Decision saved and customer notified by email.');
        if (status === 'accepted') await load();
        else setInquiries((items) => items.map((item) => item.id === id ? { ...item, status } : item));
      }
    } catch {
      setError('Network error while updating the booking request.');
    }
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[1.5fr_1fr]">
        <div>
          <div className="mb-4 flex items-center justify-between"><h2 className="text-2xl font-semibold">Booking requests</h2><button onClick={() => void load()} className="text-sm font-medium text-primary hover:underline">Refresh</button></div>
          <div className="space-y-4">
            {inquiries.length ? inquiries.map((inquiry) => (
              <article key={inquiry.id} className="rounded-2xl border bg-card p-5 shadow-sm">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm text-muted-foreground">{experienceNames.get(inquiry.experience_id)}</p><h3 className="mt-1 text-lg font-semibold">{inquiry.customer_name}</h3><p className="mt-1 text-sm">{inquiry.requested_date} · {inquiry.guests} guests</p><p className="mt-1 text-sm text-muted-foreground">{inquiry.customer_email}{inquiry.customer_phone ? ` · ${inquiry.customer_phone}` : ''}</p></div><StatusBadge status={inquiry.status} /></div>
                {inquiry.message && <p className="mt-4 rounded-xl bg-muted/60 p-3 text-sm text-muted-foreground">{inquiry.message}</p>}
                <div className="mt-5 flex flex-wrap gap-2"><button disabled={busyId === inquiry.id || inquiry.status === 'accepted'} onClick={() => void updateInquiry(inquiry.id, 'accepted')} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"><Check className="h-4 w-4" /> Accept</button><button disabled={busyId === inquiry.id || inquiry.status === 'declined'} onClick={() => void updateInquiry(inquiry.id, 'declined')} className="inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50"><X className="h-4 w-4" /> Decline</button><button disabled={busyId === inquiry.id || inquiry.status === 'contacted'} onClick={() => void updateInquiry(inquiry.id, 'contacted')} className="rounded-xl border px-4 py-2 text-sm font-semibold disabled:opacity-50">Mark contacted</button></div>
              </article>
        ...
        <div>
          ...
          <form onSubmit={saveAvailability} className="mt-4 space-y-4 rounded-2xl border bg-card p-5 shadow-sm">
            <label className="block text-sm font-medium">Experience<select value={availabilityExperienceId} onChange={(event) => setAvailabilityExperienceId(event.target.value)} className="mt-1 w-full rounded-xl border bg-background px-3 py-2">{experiences.map((experience) => <option key={experience.id} value={experience.id}>{experience.title}</option>)}</select></label>
            <label className="block text-sm font-medium">Date<input type="date" min={new Date().toISOString().slice(0, 10)} value={availabilityDate} onChange={(event) => setAvailabilityDate(event.target.value)} className="mt-1 w-full rounded-xl border bg-background px-3 py-2" /></label>
            <label className="block text-sm font-medium">Status<select value={availabilityStatus} onChange={(event) => setAvailabilityStatus(event.target.value as AvailabilityStatus)} className="mt-1 w-full rounded-xl border bg-background px-3 py-2"><option value="available">Available</option><option value="on_request">On request</option><option value="sold_out">Sold out</option><option value="blocked">Blocked</option></select></label>
            <button disabled={busyId === 'availability' || !availabilityExperienceId} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 font-semibold text-primary-foreground disabled:opacity-50">{busyId === 'availability' && <Loader2 className="h-4 w-4 animate-spin" />} Save availability</button>
          </form>
          <div className="mt-6 space-y-2"><h3 className="font-semibold">Upcoming overrides</h3>{availability.length ? availability.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl border bg-card p-3 text-sm"><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-muted-foreground" /><span>{item.service_date}</span></div><span className="capitalize text-muted-foreground">{item.status.replace('_', ' ')}</span></div>) : <p className="text-sm text-muted-foreground">No upcoming overrides. Experiences remain request-to-book by default.</p>}</div>
        </div>
      </section>
    </main>
  );
