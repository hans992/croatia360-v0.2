...
export default function OperatorBookingsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    const { data, error: loadError } = await supabase
      .from('bookings')
      .select('id,experience_id,booking_reference,customer_name,service_date,guests,total_cents,currency,status,payment_status,quote_status')
  ...
  async function saveQuote(booking: Booking) {
    ...
    const { error: quoteError } = await supabase.rpc('operator_quote_booking', {
      ...
      p_total_cents: Math.round(amount * 100),
    });
  ...
  if (loading) {
    return <section className="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8"><div className="flex items-center gap-2 border-t pt-8 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /> Loading bookings…</div></section>;
  }
