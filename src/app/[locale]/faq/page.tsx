...
import { getFaqItems } from "@/lib/faqItems";
...
export default async function FaqPage(props: FaqPageProps) {
  ...
  if (resolvedParams && typeof resolvedParams.locale === "string" && appLocalesStringArray.includes(resolvedParams.locale)) {
    ...
    console.warn(`[faq/page.tsx] Invalid or unsupported locale '${resolvedParams?.locale}'. Using fallback: ${fallbackLng}`);
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16 max-w-3xl">
      <div className="mb-12 md:mb-16">
        <p className="mb-2 text-sm font-medium text-primary">{BRAND.name}</p>
        <h1 className="section-title mb-2">Frequently asked questions</h1>
        <p className="text-body-lg text-muted-foreground">Booking requests, operator confirmation, payments and choosing the right boat experience.</p>
      </div>
      <section aria-label={`${BRAND.name} frequently asked questions`}>
        <FaqAccordion items={getFaqItems(effectiveLocale)} />
      </section>
    </div>
  );
}
