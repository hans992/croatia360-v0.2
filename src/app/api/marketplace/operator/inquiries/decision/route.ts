...
export async function POST(request: NextRequest) {
  ...
  if (!supabaseUrl || !anonKey) {
    return NextResponse.json({ error: 'Operator actions are not configured.' }, { status: 503 });
  }
  ...
  if (userError || !userData.user) {
    return NextResponse.json({ error: 'Invalid operator session.' }, { status: 401 });
  }
  ...
  const { data: context, error: contextError } = await supabase
    ...
    .select(`
      ...
      customer_email,
      requested_date,
      experiences!booking_inquiries_experience_id_fkey(
        title,
  ...
  if (contextError || !context) {
    return NextResponse.json({ error: 'Booking request not found or access denied.' }, { status: 404 });
  }
  ...
  if (payload.decision !== 'contacted') {
    ...
    const experienceTitle = row.experiences?.title ?? 'Croatia360 experience';
    const operatorName = row.experiences?.operators?.name ?? 'Croatia360 operator';

    try {
      const emailResult = await sendCustomerDecisionEmail({
      ...
      if (!emailResult.sent) notificationWarning = 'Decision saved, but customer email is not configured.';
    } catch (error) {
