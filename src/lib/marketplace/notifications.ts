...
export async function sendOperatorNewInquiryEmail(input: {
  to: string;
  operatorName: string;
  ...
  await transporter.sendMail({
    from: `"Croatia360" <${smtp.user}>`,
    to: input.to,
    replyTo: input.customerEmail,
    subject: `New Croatia360 booking request: ${input.experienceTitle}`,
    text: [
      `Hi ${input.operatorName},`,
      '',
      `You received a new booking request for ${input.experienceTitle}.`,
      ...
      '',
      'Open the Croatia360 operator portal to accept, decline or contact the customer.',
    ].filter(Boolean).join('\n'),
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#172033;max-width:640px;margin:0 auto">
        <p style="font-size:13px;color:#64748b">Croatia360 operator notification</p>
        <h1 style="font-size:24px;margin:0 0 16px">New booking request</h1>
        <p>Hi ${escapeHtml(input.operatorName)},</p>
        <p>You received a new request for <strong>${escapeHtml(input.experienceTitle)}</strong>.</p>
        <div style="margin:20px 0;padding:16px;border:1px solid #e2e8f0;border-radius:12px">
          Date: <strong>${escapeHtml(input.requestedDate)}</strong><br />
          Guests: <strong>${input.guests}</strong><br />
          Customer: ${escapeHtml(input.customerName)}<br />
          Email: ${escapeHtml(input.customerEmail)}<br />
          Phone: ${escapeHtml(input.customerPhone || 'Not provided')}
        </div>
        ${input.message ? `<p><strong>Message</strong></p><p style="white-space:pre-wrap">${escapeHtml(input.message)}</p>` : ''}
        <p>Open the Croatia360 operator portal to accept, decline or contact the customer.</p>
      </div>
    `,
  });
...
export async function sendCustomerDecisionEmail(input: {
  to: string;
  customerName: string;
  experienceTitle: string;
  requestedDate: string;
  ...
  await transporter.sendMail({
    ...
    to: input.to,
    subject: accepted
      ? `Your Croatia360 request was accepted: ${input.experienceTitle}`
    ...
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#172033;max-width:620px;margin:0 auto">
        <p style="font-size:13px;color:#64748b">Croatia360 booking request</p>
